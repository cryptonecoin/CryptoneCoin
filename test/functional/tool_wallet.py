#!/usr/bin/env python3
# Copyright (c) 2018-2019 The Cryptonecoin Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.
"""Test cryptonecoin-wallet."""
...
class ToolWalletTest(CryptonecoinTestFramework):
    ...
    def cryptonecoin_wallet_process(self, *args):
        binary = self.config["environment"]["BUILDDIR"] + '/src/cryptonecoin-wallet' + self.config["environment"]["EXEEXT"]
        args = ['-datadir={}'.format(self.nodes[0].datadir), '-regtest'] + list(args)
        return subprocess.Popen([binary] + args, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True)

    def assert_raises_tool_error(self, error, *args):
        p = self.cryptonecoin_wallet_process(*args)
    ...
    def test_invalid_tool_commands_and_args(self):
        ...
        # `cryptonecoin-wallet help` raises an error. Use `cryptonecoin-wallet -help`.
