#!/usr/bin/env python3
# Copyright (c) 2018 The Cryptonecoin Core developers
...
class AvoidReuseTest(CryptonecoinTestFramework):
    ...
    def test_fund_send_fund_senddirty(self):
        '''
        Test the same as test_fund_send_fund_send, except send the 10 CTC with
        the avoid_reuse flag set to false. This means the 10 CTC send should succeed,
    ...
    def test_fund_send_fund_send(self):
        ...
        [0] sends 10 CTC to A.
        [1] spends 5 CTC from A. (leaving roughly 5 CTC useable)
        [0] sends 10 CTC to A again.
        [1] tries to spend 10 CTC (fails; dirty).
        [1] tries to spend 4 CTC (succeeds; change address sufficient)
        '''
